...
Range sliders are best suited for quickly selecting an interval within a designated range. They offer an intuitive way to adjust settings like volume, price filters, or relative distances.

Sliders are available in two variants:

- [Single slider](/components/slider/overview.md)
- Range slider

## Anatomy

::: image-block
![Example of a Range slider](/components/range-slider/overview-anatomy.svg)
:::
...
1. **Text field suffix (optional)**: Provides contextual clarification for values when their meaning is ambiguous, such as currency or measurement units.
