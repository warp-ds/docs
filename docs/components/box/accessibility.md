...
<DoDont>
<Do imgurl="/docs/components/box/placeholders/a11y-role-do.svg" imgalt="A purely decorative box marked with role equals empty string, so no landmark is exposed.">

Set `role=""` when the box is purely visual grouping.

**Why**: An unnamed region landmark adds a wrapper that most screen readers do not announce and that clutters the landmark list.

</Do>
<Do not imgurl="/docs/components/box/placeholders/a11y-role-dont.svg" imgalt="A page with several boxes, each exposing an unnamed region landmark in the landmarks list.">

Leave the default `role="region"` on every box on the page.

**Why**: Multiple unnamed regions dilute the landmark list, making real landmarks harder to find.

</Do>
<Do imgurl="/docs/components/box/placeholders/a11y-naming-do.svg" imgalt="A box with a visible heading referenced by aria-labelledby, naming the region.">

Give the box a visible heading and, if you keep the region role, point `aria-labelledby` at it.

**Why**: A visible name helps every user, and it names the landmark at the same time.

</Do>
<Do not imgurl="/docs/components/box/placeholders/a11y-naming-dont.svg" imgalt="An aria-label set on the w-box host element, not reaching the role on the shadow DOM div inside.">

Use `aria-label` on `<w-box>` in Elements to name the region.

**Why**: The role sits on a div inside the shadow root, so the label on the host never reaches it. The region stays unnamed.

</Do>
<Do imgurl="/docs/components/box/placeholders/a11y-interactive-do.svg" imgalt="A button inside a box showing a clearly visible focus ring.">

Keep interactive content inside the box focusable and in DOM order.

**Why**: Keyboard users move through the page in DOM order. A mismatch between visual and DOM order is disorienting.

</Do>
<Do not imgurl="/docs/components/box/placeholders/a11y-interactive-dont.svg" imgalt="A whole box surface turned into a click target, with a cursor over it and no focus indicator.">

Add `tabindex` or a click handler to the box to make the whole surface actionable.

**Why**: The box has no role, no focus indicator, and no keyboard activation. Use [Card](/components/card/overview.md), which is built to be interactive.
...
- **VoiceOver (macOS/iOS)**: navigate through the box with <kbd>VO</kbd> + arrow keys. Confirm the content is announced in a sensible order and that no unnamed region is announced. Open the rotor and check the landmark list.
- **TalkBack (Android)**: swipe through the box and confirm each part is announced, including a content description for any meaningful icon.
- **NVDA or JAWS (Windows)**: press <kbd>D</kbd> to cycle landmarks and confirm the box only appears there when you intend it to.

**Keyboard**

- Tab through the page and confirm the box is skipped while every link and button inside it is reachable.
- Confirm each interactive element shows a clearly visible focus indicator against the box surface — check the bordered variant in particular, where the surface differs from the page background.

**Visual**

- Check text and interactive elements against the box surface with a contrast checker, in light and dark themes and in every brand you ship.
- Zoom to 200% and confirm nothing is clipped or overlapping.
- Check the bleed variant below and above the 480px breakpoint.

**Automated**

- Run axe DevTools or Lighthouse on pages using Box. Automated tools reliably catch unnamed landmarks and contrast failures — the two most common Box issues — but cannot tell you whether the grouping makes sense. Review that manually.
